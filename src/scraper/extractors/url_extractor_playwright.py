...
class PlaywrightExtractor:
        ...
        while waited < max_wait and not challenge_complete:
            try:
                ...
                try:
                    ...
                    if not (
                        "just a moment" in current_title.lower() or "checking your browser" in current_title.lower()
                    ):
    ...
    def _detect_pagination(self, page: Any, toc_url: str) -> List[str]:
        ...
        try:
            ...
            for link in all_links:
                ...
                if re.search(r"/\d+$", normalized_href):
                    href_base = re.sub(r"/\d+$", "", normalized_href.rstrip("/"))
                    if href_base.lower() == base_path.lower() or (
                        href_base.lower() in base_path.lower() and len(href_base) >= len(base_path) - 5
                    ):
        ...
        for link in pagination_links:
            ...
            if href:
                ...
                is_pagination_url: bool = (
                    ...
                    or "/page/" in url_lower
                    or "/p/" in url_lower
                    or bool(re.search(r"[?&]p=\d+", url_lower))
                    or (bool(re.search(r"/\d+$", url_lower)) and not is_chapter_url(full_page_url, link_text))
                )
        ...
        if len(page_urls_to_visit) < 10 and len(pagination_links) > 20:
            page_urls_to_visit.extend(
                self._construct_pagination_urls_from_links(page, toc_url, pagination_links, base_toc, seen_page_urls)
            )
    ...
    def _construct_pagination_urls_from_links(
        self, page: Any, toc_url: str, pagination_links: List[Any], base_toc: str, seen_page_urls: set[str]
    ) -> List[str]:
        ...
        for link in pagination_links:
            ...
            if href:
                ...
                page_match = re.search(
                    r"[?&]page[=_](\d+)|/page[/-](\d+)|/p[/-](\d+)|page[=_](\d+)|p[=_](\d+)", normalized_href
                )
                if page_match:
                    page_num: int = int(
                        page_match.group(1)
                        or page_match.group(2)
                        or page_match.group(3)
                        or page_match.group(4)
                        or page_match.group(5)
                    )
                    if 1 <= page_num <= 200:
    ...
    def _extract_via_pagination(
        self, page: Any, toc_url: str, page_urls: List[str], should_stop: Optional[Callable[[], bool]] = None
    ) -> List[str]:
        ...
        for idx, page_url in enumerate(page_urls[:max_pages_to_visit], 1):
                    ...
                    while waited < max_wait:
                        ...
                        try:
                            ...
                            if not (
                                ...
                                or "checking your browser" in current_title.lower()
                            ):
                                break
                        except Exception as e:
                            pass
            ...
            try:
                ...
                logger.info(
                    f"Page {idx}/{total_pages}: Found {len(page_chapters)} chapters (total so far: {len(all_chapter_urls)})"
                )
            except Exception as e:
        ...
        logger.info(
            f"✓ Playwright found {len(unique_urls)} unique chapter URLs from {len(page_urls[:max_pages_to_visit]) + 1} pages"
        )
        page.close()  # type: ignore[attr-defined]
        return unique_urls

    def _try_fallback_pagination_detection(
        self, page: Any, toc_url: str, min_chapter_number: Optional[int]
    ) -> List[str]:
        ...
        logger.info(
            f"Found {current_chapter_count} chapters but need {min_chapter_number}, trying to construct pagination URLs..."
        )
