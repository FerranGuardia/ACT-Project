...
class TTSEngine:
    ...
    def generate_voice_preview(
        ...
        pitch: Optional[float] = None,
        volume: Optional[float] = None,
        sample_text: str = "This is a preview of the selected voice."
    ) -> Optional[Path]:
        """
        Generate a voice preview audio file for UI testing.
        
        This method is intended for UI integration to allow users to preview
        voices before committing to full conversion.
        
        Args:
            output_path: Path for preview audio file. If None, uses temp file
            voice: Voice short name. If None, uses config
            rate: Speech rate. If None, uses config
            pitch: Pitch adjustment. If None, uses config
            volume: Volume adjustment. If None, uses config
            sample_text: Text to use for preview (default: short sample)
        ...
        if output_path is None:
            temp_dir = Path(tempfile.gettempdir()) / "act_tts_preview"
            temp_dir.mkdir(parents=True, exist_ok=True)
            output_path = temp_dir / "voice_preview.mp3"
        ...
        if success:
            logger.debug(f"Voice preview generated: {output_path}")
            return output_path
        else:
            logger.warning("Failed to generate voice preview")
            return None
