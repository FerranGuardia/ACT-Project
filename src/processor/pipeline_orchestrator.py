...
class PipelineOrchestrator:
    ...
    def _process_all_chapters_impl(
        ...
        # Save processing metadata to output folder
        self._save_processing_metadata(result)

        logger.info(f"Processing complete: {completed} completed, {failed} failed")
        return result

    def _save_processing_metadata(self, result: Dict[str, Any]) -> None:
        """
        ...
        try:
            ...
            metadata_dir = self.file_manager.get_metadata_dir()
            ...
            processing_metadata = {
                "processing_summary": {
                    ...
                    "novel_title": self.project_manager.get_metadata().get("novel_title", self.project_name),
