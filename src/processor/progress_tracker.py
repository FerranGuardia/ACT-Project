...
class ProgressTracker:
    ...
    def get_overall_progress(self) -> float:
        ...
        completed = sum(1 for status in self.chapter_statuses.values() if status == ProcessingStatus.COMPLETED)
    ...
    def update_chapter(self, chapter_num: int, status: ProcessingStatus, message: Optional[str] = None) -> None:
        """
    ...
    def get_summary(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "pending": sum(1 for status in self.chapter_statuses.values() if status == ProcessingStatus.PENDING),
            "in_progress": sum(
                1
                for status in self.chapter_statuses.values()
                if status in [ProcessingStatus.SCRAPING, ProcessingStatus.EDITING, ProcessingStatus.CONVERTING]
            ),
