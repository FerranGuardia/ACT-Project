...
class ModeConfig:
    ...
    def create_card(self, callback: Callable[[str], None]):
        ...
        return GenreCard(
            title=self.title, description=self.description, icon=self.icon, callback=lambda: callback(self.id)
        )


# Mode configurations - explicitly typed as Final for static analysis
MODES_CONFIG: Final[list[ModeConfig]] = [
    ModeConfig(id="scraper", title="Scraper", description="Extract text content from webnovels and stories", icon="📖"),
    ModeConfig(
        id="tts", title="Text-to-Speech", description="Convert text files into natural-sounding audio", icon="🎙️"
    ),
    ModeConfig(
        id="merger", title="Audio Merger", description="Combine multiple audio files into seamless chapters", icon="🔊"
    ),
    ModeConfig(
        id="full_auto",
