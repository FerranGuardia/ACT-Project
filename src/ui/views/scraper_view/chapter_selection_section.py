...
from ui.styles import get_radio_button_style, get_spin_box_style, get_line_edit_style, get_group_box_style, COLORS
...
class ChapterSelectionSection(QGroupBox):
    ...
    def get_chapter_selection(self) -> Dict[str, Any]:
        ...
        if self.all_chapters_radio.isChecked():
            ...
            return {"type": "range", "from": self.from_spin.value(), "to": self.to_spin.value()}
        else:  # specific
            chapters = [int(x.strip()) for x in self.specific_input.text().split(",")]
            return {"type": "specific", "chapters": chapters}
