...
from ui.styles import (
    get_font_family,
    get_font_size_large,
    get_icon_container_style,
    get_queue_item_style,
    get_secondary_text_style,
...
class BaseQueueItemWidget(QWidget):
    ...
    def setup_ui(self) -> None:
        ...
        icon_label.setMinimumSize(ViewConfig.QUEUE_ITEM_ICON_SIZE, ViewConfig.QUEUE_ITEM_ICON_SIZE)
    ...
    def update_status(self, status: str, progress: int = 0) -> None:
        ...
        if self.status == "Processing":
            if self.progress_bar not in self.findChildren(
                QProgressBar, options=Qt.FindChildOption.FindDirectChildrenOnly
            ):
