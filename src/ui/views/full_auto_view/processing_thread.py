...
class ProcessingThread(QThread):
    ...
    def __init__(
        ...
        chapter_selection: Optional[Dict[str, Any]] = None,
    ...
    def _run_gap_detection(
        self, pipeline: ProcessingPipeline, start_from: int, end_chapter: Optional[int]
    ) -> list[int]:
        """
        ...
        try:
            ...
            # Create gap detector
            gap_detector = GapDetector(project_manager=pipeline.project_manager, file_manager=pipeline.file_manager)
            ...
            if missing_chapters:
                ...
                self.status.emit(f"Found {len(missing_chapters)} missing chapters - will re-scrape")
            else:
    ...
    def run(self):
        ...
        try:
            ...
            if not self.pipeline.initialize_project(novel_url=self.url, toc_url=self.url, novel_title=self.novel_title):
            ...
            missing_chapters = self._run_gap_detection(
                pipeline=self.pipeline, start_from=start_from, end_chapter=end_chapter
            )
