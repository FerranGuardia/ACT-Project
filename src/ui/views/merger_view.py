        ...
        try:
            ...
            try:
                ...
                if not ffmpeg_path:
                    self.finished.emit(
                        False,
                        "ffmpeg not found. pydub requires ffmpeg to process audio files.\nPlease install ffmpeg: https://ffmpeg.org/download.html",
                    )
                    return
            ...
            for idx, file_path in enumerate(self.file_paths):
                ...
                try:
                    ...
                    if (
                        ...
                        or "avconv" in error_msg.lower()
                        or "ffprobe" in error_msg.lower()
                        or error_type == "FileNotFoundError"
                    ):
                        if (
                            "ffmpeg" in error_msg.lower()
                            or "avconv" in error_msg.lower()
...
class MergerView(QWidget):
    ...
    def add_files(self):
        ...
        files, _ = QFileDialog.getOpenFileNames(
            self, "Select Audio Files", "", "Audio Files (*.mp3 *.wav *.ogg *.m4a *.flac);;All Files (*.*)"
        )
    ...
    def start_merging(self):
        ...
        self.merger_thread = AudioMergerThread(self.file_paths.copy(), output_path, silence_duration)
    ...
    def browse_output_file(self):
        ...
        file_path, _ = QFileDialog.getSaveFileName(self, "Save Merged Audio", "", "Audio Files (*.mp3 *.wav *.ogg)")
        if file_path:
