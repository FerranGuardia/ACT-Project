...
class TestAsyncArchitecture:
    ...
    def test_async_availability_check(self, mock_edge_tts):
    ...
    @patch('tts.providers.edge_tts_provider.edge_tts')
    def test_async_voice_loading(self, mock_edge_tts):
    ...
    @patch('tts.providers.edge_tts_provider.edge_tts')
    def test_async_conversion_wrapper(self, mock_edge_tts):
    ...
    def test_no_event_loop_leaks(self):
        ...
        initial_loops = len(asyncio._get_running_loop())

        # Create provider (triggers async operations)
        provider = EdgeTTSProvider()

        # Should not have leaked event loops
        final_loops = len(asyncio._get_running_loop())
...
class TestAsyncIntegration:
    ...
    @patch('tts.providers.edge_tts_provider.edge_tts')
    def test_async_chunk_conversion(self, mock_edge_tts):
...
class TestAsyncErrorScenarios:
    ...
    @patch('tts.providers.edge_tts_provider.edge_tts')
    def test_async_import_error_handling(self, mock_edge_tts):
        """Test handling of edge-tts import errors"""
        # Mock import error
        mock_edge_tts.list_voices.side_effect = ImportError("No module named 'edge_tts'")
    ...
    @patch('tts.providers.edge_tts_provider.edge_tts')
    def test_async_network_error_handling(self, mock_edge_tts):
        """Test handling of network errors in async operations"""
        # Mock network-related errors
        mock_edge_tts.list_voices.side_effect = asyncio.TimeoutError("Connection timed out")

        voices = self.provider.get_voices()
        assert voices == []  # Should handle timeout gracefully

    @patch('tts.providers.edge_tts_provider.edge_tts')
    def test_async_conversion_error_recovery(self, mock_edge_tts):
